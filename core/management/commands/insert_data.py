...
class Command(BaseCommand):
    ...
    def xls_reader(self):
        data = xlrd.open_workbook('/Users/ryuchen/GitHub/Bistu-backend/data/2019.xlsx')
        ...
        for r in range(2, rows_num - 1):
            ...
            self.classes.add(values[11])
            self.majors.add((values[5], values[6]))
            self.academies.add(values[15])
            student = dict(zip(keys, values))
            self.students.append(student)

    def fake_data(self):
        self.clear_all_data()  # clean all data

        fake = Faker('zh_CN')

        self.xls_reader()  # xls data reader
        ...
        # 生成学院管理员账户
        staff_user_list = []
        for _ in range(len(self.academies)):
        ...
        for _ in self.students:
            student = User(
                ...
                first_name=_['姓名'][:1],
                last_name=_['姓名'][1:],
                email='{}@bistu.edu.cn'.format(''.join(lazy_pinyin(_['姓名']))),
                is_active=False
            )
        ...
        for _ in self.students:
            stu_political = ''
            for tag in PoliticalType:
                if _['政治面貌'] == tag.value:
                    stu_political = tag.name
            student = Student(
                stu_number=int(_['学号']),
                stu_gender='G1' if _['性别'] == '男' else 'G2',
                stu_card_type='身份证',
                stu_cardID=_['证件号码'],
                stu_candidate_number=_['考生编号'],
                stu_nation=_['民族码'],
                stu_source=fake.random.choice([x[1] for x in ProvinceOfChina]),
                stu_is_village=fake.random.choice([True, False]),
                stu_political=stu_political,  # 政治面貌
                stu_type=fake.random.choice([tag.name for tag in StudentCategoryType]),  # 学生类型
                stu_learn_type='S1',  # 学习类型
                stu_learn_status='S2',  # 学习阶段
                stu_grade=1,  # 年级
                stu_system=3,
                stu_cultivating_mode=fake.random.choice([tag.name for tag in CultivatingModeType]),
                stu_enrollment_category=fake.random.choice([tag.name for tag in EnrollmentCategoryType]),
                stu_nationality='中国',
                stu_special_program=fake.random.choice([tag.name for tag in SpecialProgramType]),
                ...
                stu_gain_diploma=False,
                stu_gain_cert=False,
                stu_telephone=self.create_telephone(),
                stu_status=fake.random.choice([tag.name for tag in StudentStatusType]),
                stu_mid_check=fake.random.choice([tag.name for tag in MidCheckType]),
            )

            student.stu_user = User.objects.filter(username=int(_['学号']), is_active=False).first()
            student.stu_name = student.stu_user.first_name + student.stu_user.last_name
            # 学生的学院/专业/班级信息
            student.stu_academy = Academy.objects.filter(aca_cname=_['学院']).first()   # 学院
            student.stu_major = Major.objects.filter(maj_code=_['专业代码']).first()
        ...
        #         edu_study_major=fake.random.choice(major_list).maj_name,
        #         edu_study_field=fake.random.choice(research_list).res_name
        #     )
        #
        #     teacher = Tutor(
        #         tut_number=random.randint(197901010000, 201901010000),
        #         tut_gender=fake.random.choice([tag.name for tag in GenderType]),
        #         tut_title=fake.random.choice([tag.name for tag in TitleType]),
        #         tut_cardID=self.create_card_id(),
        #         tut_birth_day=fake.date_of_birth(minimum_age=32, maximum_age=65),
        #         tut_entry_day=fake.date(),
        #         tut_political=fake.random.choice([tag.name for tag in PoliticalType]),
        #         tut_telephone=self.create_telephone(),
        #         tut_degree=fake.random.choice([tag.name for tag in DegreeType]),
        #     )
        #     avatar_name = fake.random.choice(Avatars)
        #     teacher.tut_avatar.save(avatar_name, File(
        #         open(os.path.join(settings.MEDIA_ROOT, "avatar", fake.random.choice(Avatars)), "rb")))
        #     teacher.tut_user = teacher_list.pop()
        #     teacher.tut_name = teacher.tut_user.first_name + teacher.tut_user.last_name
        #     teacher.tut_education = education
        #     teacher.tut_academy = fake.random.choice(academy_list)
        #     teacher.save()
        ...
        #             stu_gender=fake.random.choice([tag.name for tag in GenderType]),
        #             stu_card_type='身份证',
        ...
        #             stu_source=fake.random.choice([x[1] for x in ProvinceOfChina]),
        #             stu_is_village=fake.random.choice([True, False]),
        #             stu_political=fake.random.choice([tag.name for tag in PoliticalType]),  # 政治面貌
        #             stu_type=fake.random.choice([tag.name for tag in StudentCategoryType]),  # 学生类型
        #             stu_learn_type=fake.random.choice([tag.name for tag in StudentLearnType]),  # 学习类型
        #             stu_learn_status=fake.random.choice([tag.name for tag in DegreeType]),  # 学习阶段
        ...
        #             stu_mid_check=fake.random.choice([tag.name for tag in MidCheckType]),
        #         )
        #         student.stu_user = student_list.pop()
        #         student.stu_name = student.stu_user.first_name + student.stu_user.last_name
        ...
        #         stu_major = fake.random.choice(stu_academy.aca_majors.all())
        #         student.stu_major = stu_major  # 专业
        #         student.stu_research = fake.random.choice(stu_major.maj_research.all())
        #
        #         Class.objects.get_or_create(
        ...
        #
        #             student.stu_mid_check = fake.random.choice([tag.name for tag in MidCheckType])  # 中期考核成绩
